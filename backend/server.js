...
// ----- DEPENDENCIES -----------------
const express  = require('express');                //Bring in the Express framework
const logger = require('morgan');                   //For logging and debugging
const bodyParser = require('body-parser');          //Easy request body parsing
...
app.use(function (err, req, res, next) {
	if (err.name === 'UnauthorizedError') {
		//Send back Angular app
		res.status(401).sendFile(path.join(__dirname + '/app/index.html'));
	}
});
